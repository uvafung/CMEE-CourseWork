# Name: SDM_HOG_citisci_AllYear_model_evaluation_3.R
...
enviro <- enviro %>% subset(enviro$HOGns %in% c(0,1)) 
...
ggplot(enviro, aes(AMN750, id)) + geom_point() # % Amenity cover
ggplot(enviro, aes(CEM1000, id)) + geom_point() # % Cemetery cover
ggplot(enviro, aes(PLPK100, id)) + geom_point() # Park & play-space
ggplot(enviro, aes(SPRT1000, id)) + geom_point() # Sport cover
ggplot(enviro, aes(WAT100, id)) + geom_point() # Water cover 
ggplot(enviro, aes(WD100, id)) + geom_point() # Woodland
ggplot(enviro, aes(TRAF100, id)) + geom_point() # Traffic
ggplot(enviro, aes(IMP100, id)) + geom_point() # Impervious cover
ggplot(enviro, aes(DTCH100, id)) + geom_point() # Detached housing 
ggplot(enviro, aes(SEMI500, id)) + geom_point() # Semi-detached housing
ggplot(enviro, aes(TERR750, id)) + geom_point() # Terraced housing
ggplot(enviro, aes(DWL100, id)) + geom_point() # Housing density
ggplot(enviro, aes(HD100, id)) + geom_point() # Human population density
ggplot(enviro, aes(BadgCS100, id)) + geom_point() # Badger presence
ggplot(enviro, aes(GDN100, id)) + geom_point() # Garden cover
ggplot(enviro, aes(FoxCS100, id)) + geom_point() # Fox presence

# Log transformation applied to human pop density, housing density and traffic
ggplot(enviro, aes(logHD100, id)) + geom_point() # log Human population density
ggplot(enviro, aes(logDWL100, id)) + geom_point() # log housing density
ggplot(enviro, aes(logTRAF100, id)) + geom_point() # log Traffic
...
enviro_scaled <- enviro[, c('ALT1000', 'AMN750', 'CEM1000', 'PLPK100', 'SPRT1000', 
                            'WAT100', 'WD100', 'TRAF100', 'IMP100', 'DTCH100', 
                            'SEMI500', 'TERR750', 'logDWL100', 'logHD100', 
                            'BadgCS100', 'GDN100', 'FoxCS100')]
...
# Remove predictor with highest vif (logDWL100)
enviro_var16 <- enviro[, c('ALT1000', 'AMN750', 'CEM1000', 'PLPK100', 'SPRT1000', 
                           'WAT100', 'WD100', 'TRAF100', 'IMP100', 'DTCH100', 
                           'SEMI500', 'TERR750', 'logHD100', 
                           'BadgCS100', 'GDN100', 'FoxCS100')]
...
glm_vifchecked <- glm(HOGns ~  
                        GDN100 + ALT1000 + AMN750 + CEM1000 + PLPK100 + SPRT1000 + 
                        WAT100 + WD100 + IMP100 + DTCH100 + SEMI500 + TERR750 + 
                        logHD100 + TRAF100 + BadgCS100 + FoxCS100, 
...
glm_qua <- glm(HOGns ~  
                 GDN100 + ALT1000 + AMN750 + CEM1000 + PLPK100 + SPRT1000 + 
                 WAT100 + WD100 + poly(IMP100, 2) + DTCH100 + SEMI500 + TERR750 + 
                 poly(logHD100, 2) + TRAF100 + BadgCS100 + FoxCS100, 
                      family = binomial(link="logit"), data = enviro)
...
for (i in 1:100){  # i = 100 repeat split sample cross validation
  ...
  glm.fit_GLM = glm(HOGns ~  # simple GLM
                      GDN100 + ALT1000 + AMN750 + CEM1000 + PLPK100 + SPRT1000 + 
                      WAT100 + WD100 + IMP100 + DTCH100 + SEMI500 + TERR750 + 
                      logHD100 + TRAF100 + BadgCS100 + FoxCS100, 
                    family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_GLMmod[1, i] <- auc(eval$HOGns, predict(glm.fit_GLM, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  glm.fit_GLM_Qua = glm(HOGns ~  # GLM with quadratic variables
                          GDN100 + ALT1000 + AMN750 + CEM1000 + PLPK100 + SPRT1000 + 
                          WAT100 + WD100 + poly(IMP100, 2) + DTCH100 + SEMI500 + TERR750 + 
                          poly(logHD100, 2) + TRAF100 + BadgCS100 + FoxCS100,
                        family='binomial'(link = 'logit'), data = calib) # calib
  auc_GLMmod[2, i] <- auc(eval$HOGns, predict(glm.fit_GLM_Qua, eval, type='response')) #eval
  
  
  glm.fit_GLM_Qua_selected <- glm(HOGns ~  # GLM with selected quadratic variables after stepAIC
                                    GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
                                    poly(IMP100, 2) + DTCH100 + SEMI500 + TERR750 + 
                                    poly(logHD100, 2) + BadgCS100 + FoxCS100, 
                          family = binomial(link="logit"), data = calib)
  auc_GLMmod[3, i] <- auc(eval$HOGns, predict(glm.fit_GLM_Qua_selected, eval, type='response'))
...
for (i in 1:100) { # i = 100 repeat split sample cross validation
  ...
  GLM_tss <- ecospat.max.tss(predict(glm.fit_GLM, eval, type='response'), eval$HOGns) # use eval to predict
  tss_GLMmod[1, i] <- GLM_tss$max.TSS
  tss_GLMmod[4, i] <- GLM_tss$max.threshold
  
  GLM_Qua_tss <- ecospat.max.tss(predict(glm.fit_GLM_Qua, eval, type='response'), eval$HOGns)
  tss_GLMmod[2, i] <- GLM_Qua_tss$max.TSS
  tss_GLMmod[5, i] <- GLM_Qua_tss$max.threshold
  
  GLM_Qua_selected_tss <- ecospat.max.tss(predict(glm.fit_GLM_Qua_selected, eval, type='response'), eval$HOGns)
...
write.csv(auc_GLMmod,"../../../results/CitiSci/AllYear/Hedgehog/hog_GLM_auc.csv")
write.csv(tss_GLMmod,"../../../results/CitiSci/AllYear/Hedgehog/hog_GLM_tss.csv")
...
# load the dataset
enviro_biomod2 <- enviro[, c('Easting','Northing','HOGns', 
                             'GDN100', 'ALT1000', 'AMN750', 'PLPK100', 'WAT100',
                             'WD100', 'IMP100', 'DTCH100', 'SEMI500', 'TERR750', 
                             'logHD100', 'BadgCS100', 'FoxCS100')] # all 13 variables


# convert response variable (HOGns) to numeric
enviro_biomod2$HOGns <- as.numeric(enviro_biomod2$HOGns) 

# convert badger presence (BadgCS100) and fox presence (FoxCS100) to factor
enviro_biomod2$BadgCS100 <- as.factor(enviro_biomod2$BadgCS100) 
...
sampled_data <- enviro_biomod2 %>%
  subset(enviro_biomod2$HOGns %in% c(0,1))

# define the explanatory variables
vars = c('GDN100', 'ALT1000', 'AMN750', 'PLPK100', 'WAT100', 'WD100', 'IMP100', 
         'DTCH100', 'SEMI500', 'TERR750', 'logHD100', 'BadgCS100', 'FoxCS100')


# Format data using BIOMOD_FormatingData
biomod2_formatdata <- BIOMOD_FormatingData(resp.var = sampled_data$HOGns,
...
stargazer(eval_table,                
          summary = FALSE,
          type = "text",
          out = "../../../results/CitiSci/AllYear/Hedgehog/hog_evaluation_scores_UF.txt")


eval_table_mean_score <- eval_table %>% 
  group_by(d1, d2, d3) %>% summarize(cover = mean(scores_all)) %>% arrange(d1, d2)
...
ggsave("../../../results/CitiSci/AllYear/Hedgehog/hog_evaluation_plot_UF.pdf")
...
ggsave("../../../results/CitiSci/AllYear/Hedgehog/_varimp_UF.pdf")
...
ggsave("../../../results/CitiSci/AllYear/Hedgehog/_varimpB_UF.pdf")


# # Construct ensemble from all models with TSS > 0.5
# em_all <- BIOMOD_EnsembleModeling( modeling.output = myBiomodModelOutPut, # combines models and make ensemble predictions
#                                    chosen.models = 'all',
#                                    em.by = 'all',
#                                    eval.metric = c('TSS'),
#                                    eval.metric.quality.threshold = 0.5, # excluded models with score <0.5
#                                    models.eval.meth = c('TSS','ROC'),
#                                    prob.mean = TRUE ) # estimate mean probabilities across predictions
# 
# # Evaluate ensemble.. need to better understand why this is so much better
# get_evaluations(em_all)

# # Predition data
# full_data = fread("final_full_formatted_dataset.csv")
# full_data$HOG = as.numeric(full_data$HOG)
# full_data$Bpres05_250 = factor(full_data$Bpres05_250)
# 
# var_cols = attr(myBiomodModelOut, "expl.var.names")
# 
# # Dataset for predictions
# full_data_biomod <- BIOMOD_FormatingData(resp.var = full_data$HOG,
#                                          expl.var = full_data[, ..var_cols],#Get same set of vars
#                                          resp.xy = cbind(full_data$Easting, full_data$Northing),
#                                          resp.name = "Hedgehog")

# # Run predictions for RF models
# myBiomodProjection <- BIOMOD_Projection(
#   modeling.output = myBiomodModelOut,
#   new.env =  full_data[, ..var_cols],
#   proj.name = 'current',
...
# emBiomodProjection = BIOMOD_EnsembleForecasting( projection.output = myBiomodProjection,
#                                                  EM.output = em_all)
# 
# # Extract probs
# p_1000 = myBiomodProjection@proj@val
# em_p_1000 = emBiomodProjection@proj@val
# 
# # Convert from 0-1000 to 0-1
# # Just first RF model
# full_data$rf_p = p_1000[, , 1, ]/1000
# # Ensemble
# full_data$em_p = em_p_1000/1000
# 
# 
# fwrite(full_data, file = "final_full_formatted_dataset_with_p.csv")
# 
# 
# 
# 
...
ggsave("../../../results/CitiSci/AllYear/Hedgehog/hog_blocking_map.pdf")
...
stargazer(scores_all_blocking,                
          summary = FALSE,
          type = "text",
          out = "../../../results/CitiSci/AllYear/Hedgehog/hog_blocking_evaluation_scores_UF.txt")
...
scores_all_blocking_Testing.data <- scores_all_blocking %>% 
  group_by(Eval.metric, model) %>% summarize(score = mean(Testing.data)) 
...
ggsave("../../../results/CitiSci/AllYear/Hedgehog/hog_blocking_evaluation_plot_UF.pdf")
...
ggsave(paste0("../../../results/CitiSci/AllYear/Hedgehog/hog_blocked_varimp_UF.pdf"))
...
enviro_random_bg <- enviro_random_bg %>% 
  distinct(Easting,Northing,HOGns,
           GDN100, ALT1000, AMN750, PLPK100, WAT100, 
           WD100, IMP100, DTCH100, SEMI500, TERR750, 
           logHD100, BadgCS100, FoxCS100, .keep_all = TRUE)


# Add 1=presence in the new column based on HOGns = 1
enviro_random_bg$random_bg [enviro_random_bg$HOGns == 1] <- 1

# Check that the number of presence (1) are the same in both columns
table(enviro_random_bg$random_bg)
table(enviro_random_bg$HOGns)
...
# convert badger presence and fox presence (Bpres05_100) to factor
enviro_random_bg$BadgCS100 <- as.factor(enviro_random_bg$BadgCS100) 
enviro_random_bg$FoxCS100 <- as.factor(enviro_random_bg$FoxCS100) 

# subset only the sampled_data_random with hedgehog presence/absence records
...
eval_table_mean_random <- eval_table_random %>% 
  group_by(d1, d2) %>% summarize(cover = mean(scores_all_randomMaxent)) %>% arrange(d1, d2)
...
ggsave("../../../results/CitiSci/AllYear/Hedgehog/hog_varimp_UF.pdf")
...
# 
# CI <- as.numeric(predict(glm_qua_selected, enviro1, interval = 'confidence')) # confidence interval
# CI <- inv.logit(CI) # returns the inverse logit of values to create a 0-1 scale
# 
# # binary conversion based on presence absence
# enviro1 <- enviro1 %>% 
#   mutate(predictions) %>% # create new column to store prediction score
#   mutate(CI) %>% # create new column to store CI
...
#                       aes(x = Easting, y = Northing, 
#                           colour = CI)) +
#   scale_color_gradient(low="gold", high="medium blue",
#                        breaks=c(-5,3),
#                        labels=c("Low","High"),
#                        limits=c(-5,3) 
#   ) + 
...
#                       aes(x = Easting, y = Northing, 
#                           colour = binary)) +
#   scale_color_manual(labels = c("Absent", "Present"), values = c("blue", "red")) +
#   coord_fixed(ratio=1) +
#   theme_light() +
#   labs(color = "")
...
# 
# ############## CHECK GLM models performance via AUC ############
# auc_change_GLMmod <- as.data.frame(matrix(0, ncol = 100, nrow = 11, # create dataframe to store AUC outputs
#                                    dimnames = list(c('GDN100', 'ALT1000', 'AMN750', 'PLPK100', 'WAT100', 'WD100', 
#                                                        'poly(IMP100)','SEMI500', 'TERR750', 'poly(logHD100)',
#                                                        'BadgCS100'), NULL)))
# 
# # GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
# #   poly(IMP100, 2) + SEMI500 + TERR750 + poly(logHD100, 2) + 
# #   BadgCS100
# 
# for (i in 1:100){  # i = 100 repeat split sample cross validation
#   
...
#                            GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
#                            SEMI500 + TERR750 + poly(logHD100, 2) + 
#                            BadgCS100, 
#                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
...
#                         GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
#                         poly(IMP100, 2) + TERR750 + poly(logHD100, 2) + 
#                         BadgCS100, 
#                       family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
...
#   
#   
#   # 9 -- GLM without TERR750
#   glm_X_TERR750 = glm(HOGns ~  
#                         GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
#                         poly(IMP100, 2) + SEMI500 + poly(logHD100, 2) + 
#                         BadgCS100, 
#                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
#   auc_change_GLMmod[9, i] <- auc(eval$HOGns, predict(glm_X_TERR750, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
#   
#   
#   
#   # 10 -- GLM without poly(logHD100, 2) +
#   glm_X_polylogHD100 = glm(HOGns ~  
#                              GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
#                              poly(IMP100, 2) + SEMI500 + TERR750 + 
#                              BadgCS100, 
#                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
#   auc_change_GLMmod[10, i] <- auc(eval$HOGns, predict(glm_X_polylogHD100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
#   
#   
#   # 11 -- GLM without BadgCS100
#   glm_X_BadgCS100 = glm(HOGns ~  # GLM without BadgCS100
#                           GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
#                           poly(IMP100, 2) + SEMI500 + TERR750 + poly(logHD100, 2), 
#                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
#   auc_change_GLMmod[11, i] <- auc(eval$HOGns, predict(glm_X_BadgCS100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
#   
#   
# }
# 
