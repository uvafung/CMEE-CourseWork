# Name: SDM_BDG_citisci_AllYear_map_graph_plotting_4.R
# Description: This script is used to: 
# 1) Print prediction/CI/binary presence absence map for CitiSci AllYear Badgers
...
enviro <- enviro %>% subset(enviro$BADGERns %in% c(0,1)) # Only select presence absence entries
...
final_threshold <- 0.1016000 # calculated in step 2 script
...
# write.csv(enviroplot, "../../../results/CitiSci/AllYear/Badger/enviroplot.csv")
...
ggsave("../../../results/CitiSci/AllYear/Badger/badger_pre_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear/Badger/badger_CI_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear/Badger/badger_bi_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
auc_change_GLMmod <- as.data.frame(matrix(0, ncol = 100, nrow = 12, # create dataframe to store AUC outputs
                                   dimnames = list(c('GDN100', 'ALT1000', "AMN1000", 'PLPK500', 'WAT1000',
                                                     'IMP1000','DTCH1000', 'TERR1000', 'logHD1000', 'logTRAF1000', 
                                                     'FoxCS100', 'HogCS100'), NULL)))
...
for (i in 1:100){  # i = 100 repeat split sample cross validation
  ...
  glm_X_GDN100 = glm(BADGERns ~
                       ALT1000 + AMN1000 + PLPK500 + 
                       WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                       logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                    family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[1, i] <- auc(eval$BADGERns, predict(glm_X_GDN100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  # 2 -- GLM without ALT1000
  glm_X_ALT1000 = glm(BADGERns ~  
                        GDN100 + AMN1000 + PLPK500 + 
                        WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                        logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[2, i] <- auc(eval$BADGERns, predict(glm_X_ALT1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  # 3 -- GLM without AMN1000
  glm_X_AMN1000 = glm(BADGERns ~  
                        GDN100 + ALT1000 + PLPK500 + 
                        WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                        logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[3, i] <- auc(eval$BADGERns, predict(glm_X_AMN1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 4 -- GLM without PLPK500
  glm_X_PLPK500 = glm(BADGERns ~  # GLM without PLPK500
                        GDN100 + ALT1000 + AMN1000 + 
                        WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                        logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[4, i] <- auc(eval$BADGERns, predict(glm_X_PLPK500, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 5 -- GLM without WAT1000
  glm_X_WAT1000 = glm(BADGERns ~  # GLM without WAT1000
                        GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                        IMP1000 + DTCH1000 + TERR1000 + 
                        logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[5, i] <- auc(eval$BADGERns, predict(glm_X_WAT1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  

  # 6 -- GLM without IMP1000
  glm_X_IMP1000 = glm(BADGERns ~  # GLM without IMP1000
                        GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                        WAT1000 + DTCH1000 + TERR1000 + 
                        logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[6, i] <- auc(eval$BADGERns, predict(glm_X_IMP1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 7 -- GLM without DTCH1000 
  glm_X_DTCH1000 = glm(BADGERns ~  
                         GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                         WAT1000 + IMP1000 + TERR1000 + 
                         logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[7, i] <- auc(eval$BADGERns, predict(glm_X_DTCH1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 8 -- GLM without TERR1000
  glm_X_TERR1000 = glm(BADGERns ~  
                         GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                         WAT1000 + IMP1000 + DTCH1000 + 
                         logHD1000 + logTRAF1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[8, i] <- auc(eval$BADGERns, predict(glm_X_TERR1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  
  # 9 -- GLM without logHD1000
  glm_X_logHD1000 = glm(BADGERns ~  
                              GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                              WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                              logTRAF1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[9, i] <- auc(eval$BADGERns, predict(glm_X_logHD1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 10 -- GLM without logTRAF1000
  glm_X_logTRAF1000 = glm(BADGERns ~  # GLM without logTRAF1000
                            GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                            WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                            logHD1000 + FoxCS100 + HogCS100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[10, i] <- auc(eval$BADGERns, predict(glm_X_logTRAF1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  # 11 -- GLM without FoxCS100
  glm_X_FoxCS100 = glm(BADGERns ~  # GLM without FoxCS100
                         GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                         WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                         logHD1000 + logTRAF1000 + HogCS100, 
                        family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[11, i] <- auc(eval$BADGERns, predict(glm_X_FoxCS100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 12 -- GLM without HogCS100
  glm_X_HogCS100 = glm(BADGERns ~  # GLM without BadgCS250
                         GDN100 + ALT1000 + AMN1000 + PLPK500 + 
                         WAT1000 + IMP1000 + DTCH1000 + TERR1000 + 
                         logHD1000 + logTRAF1000 + FoxCS100, 
                        family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[12, i] <- auc(eval$BADGERns, predict(glm_X_HogCS100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
}

# best_mod_AUC <- as.numeric(auc_GLMmod['GLM_Qua_selected_AUC', 'Mean']) # store final threshold value
best_mod_AUC <- 0.8283779 # calculated in step 2 script
...
ggsave("../../../results/CitiSci/AllYear/Badger/badger_AUCchange_map.pdf", width=10, height=7.5, units="in", scale = 1)


auc_change_summary <- auc_change_GLMmod[,c("Predictor", "Mean", "AUC_Difference")]
write.csv(auc_change_summary, '../../../results/CitiSci/AllYear/Badger/badger_AUCchange_summary.csv')
...
enviro$AMN1000 <- enviro$AMN1000 * 100
enviro$PLPK500 <- enviro$PLPK500 * 100
enviro$WAT1000 <- enviro$WAT1000 * 100
enviro$IMP1000 <- enviro$IMP1000 * 100
...
# AMN1000
gge_AMN1000 <- ggpredict(glm_qua_selected_step,
                        terms = c('AMN1000 [all]'), ci.lvl = 0.95,
                        typical = 'mean', back.transform = T)
gge_AMN1000$Predictor <- "Amenity cover (%)"
...
# PLPK500
gge_PLPK500 <- ggpredict(glm_qua_selected_step, 
                         terms = c('PLPK500 [all]'), ci.lvl = 0.95, 
                         typical = 'mean', back.transform = T)
gge_PLPK500$Predictor <- "Park and play spaces (%)" 
...
# WAT1000
gge_WAT1000 <- ggpredict(glm_qua_selected_step,
                        terms = c('WAT1000 [all]'), ci.lvl = 0.95,
                        typical = 'mean', back.transform = T)
gge_WAT1000$Predictor <- "Water cover (%)"
...
# IMP1000
gge_IMP1000  <- ggpredict(glm_qua_selected_step,
                         terms = c('IMP1000  [all]'), ci.lvl = 0.95,
                         typical = 'mean', back.transform = T)
gge_IMP1000$Predictor <- "Impervious cover (%)"
...
# 
# # SEMI250  
# gge_SEMI250   <- ggpredict(glm_qua_selected_step, 
#                            terms = c('SEMI250   [all]'), ci.lvl = 0.95, 
#                            typical = 'mean', back.transform = T)
# gge_SEMI250$Predictor <- "Semi-detached housing (%)" 
...
# logHD1000
gge_logHD1000 <- ggpredict(glm_qua_selected_step, 
                          terms = c('logHD1000 [all]'), ci.lvl = 0.95, 
                          typical = 'mean', back.transform = T)
gge_logHD1000$Predictor <- "log human density (km2)" 
...
gge_all <- rbind(gge_GDN100, gge_ALT1000, gge_AMN1000, gge_PLPK500, gge_WAT1000,
                 gge_IMP1000, gge_DTCH1000, gge_TERR1000, gge_logHD1000, gge_logTRAF1000,  
                 gge_FoxCS100, gge_HogCS100)
...
ggplot(gge_all , aes(x, predicted)) +
  ...
  labs(y= "Probability of badger presence", x = "Predictor" ) +
  facet_wrap(~Predictor, scales = "free",
             strip.position="bottom") +
  theme_bw()

ggsave("../../../results/CitiSci/AllYear/Badger/badger_predictor_ggplot.pdf", width=10, height=7.5, units="in", scale = 1)
