# Name: SDM_FOX_citisci_XMamPred_model_evaluation_3.R
# Description: This script is used to: 
# 1) run SDM on CitiSci Foxes All year using Uva Fung's dataset to determine which 
...
enviro <- enviro %>% subset(enviro$FOXns %in% c(0,1)) 
...
# ggplot(enviro, aes(ALT750, id)) + geom_point() # % Allotment cover
# ggplot(enviro, aes(AMN1000, id)) + geom_point() # % Amenity cover
# ggplot(enviro, aes(CEM500, id)) + geom_point() # % Cemetery cover
# ggplot(enviro, aes(PLPK1000, id)) + geom_point() # Park & play-space
# ggplot(enviro, aes(SPRT1000, id)) + geom_point() # Sport cover
# ggplot(enviro, aes(WAT750, id)) + geom_point() # Water cover 
# ggplot(enviro, aes(WD1000, id)) + geom_point() # Woodland
# ggplot(enviro, aes(TRAF1000, id)) + geom_point() # Traffic
# ggplot(enviro, aes(IMP750, id)) + geom_point() # Impervious cover
...
# ggplot(enviro, aes(DWL500, id)) + geom_point() # Housing density
# ggplot(enviro, aes(HD250, id)) + geom_point() # Human population density
# ggplot(enviro, aes(BadgCS750, id)) + geom_point() # Badger presence
# ggplot(enviro, aes(GDN250, id)) + geom_point() # Garden cover
...
# ggplot(enviro, aes(logHD250, id)) + geom_point() # log Human population density
# ggplot(enviro, aes(logDWL500, id)) + geom_point() # log housing density
...
enviro_scaled <- enviro[, c('ALT750', 'AMN500', 'CEM500', 'PLPK100', 'SPRT1000', 
                            'WAT750', 'WD1000', 'logTRAF1000', 'IMP750', 'DTCH1000', 
                            'SEMI500', 'TERR1000', 'logDWL750', 'logHD500', 
                            'GDN1000')]
...
# Remove predictor with highest vif (logDWL750)
enviro_var16 <- enviro[, c('ALT750', 'AMN500', 'CEM500', 'PLPK100', 'SPRT1000', 
                           'WAT750', 'WD1000', 'logTRAF1000', 'IMP750', 'DTCH1000', 
                           'SEMI500', 'TERR1000', 'logHD500', 
                           'GDN1000')]
...
glm_vifchecked <- glm(FOXns ~  
                        GDN1000 + ALT750 + AMN500 + CEM500 + PLPK100 + SPRT1000 + 
                        WAT750 + WD1000 + IMP750 + DTCH1000 + SEMI500 + TERR1000 + 
                        logHD500 + logTRAF1000, 
...
glm_qua <- glm(FOXns ~  
                 GDN1000 + ALT750 + AMN500 + CEM500 + PLPK100 + SPRT1000 + 
                 WAT750 + WD1000 + poly(IMP750, 2) + DTCH1000 + SEMI500 + TERR1000 + 
                 poly(logHD500, 2) + logTRAF1000, 
               family = binomial(link="logit"), data = enviro)
...
glm_qua_selected <- glm(FOXns ~  
                          GDN1000 + ALT750 + CEM500 + PLPK100 + WD1000 + 
                          poly(IMP750, 2) + DTCH1000 + SEMI500 + logTRAF1000, 
                        family = binomial(link="logit"), data = enviro)
...
for (i in 1:100){  # i = 100 repeat split sample cross validation
  ...
  glm.fit_GLM = glm(FOXns ~  # simple GLM
                      GDN1000 + ALT750 + AMN500 + CEM500 + PLPK100 + 
                      SPRT1000 +  WAT750 + WD1000 + IMP750 + DTCH1000 + 
                      SEMI500 + TERR1000 + logHD500 + logTRAF1000, 
                    family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_GLMmod[1, i] <- auc(eval$FOXns, predict(glm.fit_GLM, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  glm.fit_GLM_Qua = glm(FOXns ~  # GLM with quadratic variables
                          GDN1000 + ALT750 + AMN500 + CEM500 + PLPK100 + 
                          SPRT1000 + WAT750 + WD1000 + poly(IMP750, 2) + DTCH1000 + 
                          SEMI500 + TERR1000 + poly(logHD500, 2) + logTRAF1000,
                        family='binomial'(link = 'logit'), data = calib) # calib
  auc_GLMmod[2, i] <- auc(eval$FOXns, predict(glm.fit_GLM_Qua, eval, type='response')) #eval
  
  
  glm.fit_GLM_Qua_selected <- glm(FOXns ~  # GLM with selected quadratic variables after stepAIC
                                    GDN1000 + ALT750 + CEM500 + PLPK100 + WD1000 + 
                                    poly(IMP750, 2) + DTCH1000 + SEMI500 + logTRAF1000, 
                                  family = binomial(link="logit"), data = calib)
  auc_GLMmod[3, i] <- auc(eval$FOXns, predict(glm.fit_GLM_Qua_selected, eval, type='response'))
...
for (i in 1:100) { # i = 100 repeat split sample cross validation
  ...
  GLM_tss <- ecospat.max.tss(predict(glm.fit_GLM, eval, type='response'), eval$FOXns) # use eval to predict
  tss_GLMmod[1, i] <- GLM_tss$max.TSS
  tss_GLMmod[4, i] <- GLM_tss$max.threshold
  
  GLM_Qua_tss <- ecospat.max.tss(predict(glm.fit_GLM_Qua, eval, type='response'), eval$FOXns)
  tss_GLMmod[2, i] <- GLM_Qua_tss$max.TSS
  tss_GLMmod[5, i] <- GLM_Qua_tss$max.threshold
  
  GLM_Qua_selected_tss <- ecospat.max.tss(predict(glm.fit_GLM_Qua_selected, eval, type='response'), eval$FOXns)
...
write.csv(auc_GLMmod, "../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_auc_GLMmod.csv")
write.csv(tss_GLMmod, "../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_tss_GLMmod.csv")
...
# load the dataset
enviro_biomod2 <- enviro[, c('Easting','Northing','FOXns', 
                             'GDN1000', 'ALT750', 'CEM500', 'PLPK100',
                             'WD1000', 'IMP750', 'DTCH1000', 'SEMI500', 'logTRAF1000')
                         ] # all 9 variables
...
# convert response variable (FOXns) to numeric
enviro_biomod2$FOXns <- as.numeric(enviro_biomod2$FOXns) 

# # convert badger presence (BadgCS750) and hedgehog presence (HogCS100) to factor
# enviro_biomod2$BadgCS750 <- as.factor(enviro_biomod2$BadgCS750) 
...
sampled_data <- enviro_biomod2 %>%
  subset(enviro_biomod2$FOXns %in% c(0,1))

# define the explanatory variables
vars = c('GDN1000', 'ALT750', 'CEM500', 'PLPK100',
         'WD1000', 'IMP750', 'DTCH1000', 'SEMI500', 'logTRAF1000')


# Format data using BIOMOD_FormatingData
biomod2_formatdata <- BIOMOD_FormatingData(resp.var = sampled_data$FOXns,
...
stargazer(eval_table,                
          summary = FALSE,
          type = "text",
          out = "../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_evaluation_scores_UF.txt")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_evaluation_plot_UF.pdf")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_varimp_UF.pdf")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_varimpB_UF.pdf")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_blocking_map.pdf")
...
stargazer(scores_all_blocking,                
          summary = FALSE,
          type = "text",
          out = "../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_blocking_evaluation_scores_UF.txt")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_blocking_evaluation_plot_UF.pdf")
...
ggsave(paste0("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_blocked_varimp_UF.pdf"))
...
enviro_random_bg <- enviro_random_bg %>% 
  distinct(Easting,Northing,FOXns,
           GDN1000, ALT750, CEM500, PLPK100,
           WD1000, IMP750, DTCH1000, SEMI500, logTRAF1000, .keep_all = TRUE)


# Add 1=presence in the new column based on FOXns = 1
enviro_random_bg$random_bg [enviro_random_bg$FOXns == 1] <- 1

# Check that the number of presence (1) are the same in both columns
table(enviro_random_bg$random_bg)
table(enviro_random_bg$FOXns)
...
# enviro_random_bg$BadgCS750 <- as.factor(enviro_random_bg$BadgCS750) 
