...
###### 3 model -- GLM with YES quadratic polynomial and YES selected variables after stepAIC #####
glm_qua_selected <- glm(BADGERns ~  
                          AMN1000 + PLPK500 + WD1000 + DTCH1000 + TERR1000 + 
                          poly(logHD1000, 2) + logTRAF1000, 
...
final_threshold <- 0.0673000 # calculated in step 2 script
...
write.csv(enviroplot, "../../../results/CitiSci/AllYear_XMammalPred/Badger/enviroplot.csv")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Badger/badger_pre_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Badger/badger_CI_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Badger/badger_bi_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
auc_change_GLMmod <- as.data.frame(matrix(0, ncol = 100, nrow = 7, # create dataframe to store AUC outputs
                                   dimnames = list(c("AMN1000", 'PLPK500', 'WD1000',
                                                     'DTCH1000', 'TERR1000', 'logHD1000', 'logTRAF1000'), NULL)))
...
for (i in 1:100){  # i = 100 repeat split sample cross validation
  ...
  # 1 -- GLM without AMN1000
  glm_X_AMN1000 = glm(BADGERns ~  
                        PLPK500 + WD1000 + DTCH1000 + TERR1000 + 
                        poly(logHD1000, 2) + logTRAF1000, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[1, i] <- auc(eval$BADGERns, predict(glm_X_AMN1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 2 -- GLM without PLPK500
  glm_X_PLPK500 = glm(BADGERns ~  # GLM without PLPK500
                        AMN1000 + WD1000 + DTCH1000 + TERR1000 + 
                        poly(logHD1000, 2) + logTRAF1000, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[2, i] <- auc(eval$BADGERns, predict(glm_X_PLPK500, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 3 -- GLM without WD1000
  glm_X_WD1000 = glm(BADGERns ~  # GLM without WD1000
                        AMN1000 + PLPK500 + DTCH1000 + TERR1000 + 
                        poly(logHD1000, 2) + logTRAF1000, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[3, i] <- auc(eval$BADGERns, predict(glm_X_WD1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  

  
  # 4 -- GLM without DTCH1000 
  glm_X_DTCH1000 = glm(BADGERns ~  
                         AMN1000 + PLPK500 + WD1000 + TERR1000 + 
                         poly(logHD1000, 2) + logTRAF1000, 
                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[4, i] <- auc(eval$BADGERns, predict(glm_X_DTCH1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
 
  
  # 5 -- GLM without TERR1000
  glm_X_TERR1000 = glm(BADGERns ~  
                         AMN1000 + PLPK500 + WD1000 + DTCH1000 + 
                         poly(logHD1000, 2) + logTRAF1000, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[5, i] <- auc(eval$BADGERns, predict(glm_X_TERR1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  
  # 6 -- GLM without poly(logHD1000)
  glm_X_polylogHD1000 = glm(BADGERns ~  
                              AMN1000 + PLPK500 + WD1000 + DTCH1000 + TERR1000 + 
                              logTRAF1000, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[6, i] <- auc(eval$BADGERns, predict(glm_X_polylogHD1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 7 -- GLM without logTRAF1000
  glm_X_logTRAF1000 = glm(BADGERns ~  # GLM without logTRAF1000
                            AMN1000 + PLPK500 + WD1000 + DTCH1000 + TERR1000 + 
                            poly(logHD1000, 2), 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[7, i] <- auc(eval$BADGERns, predict(glm_X_logTRAF1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  # # 10 -- GLM without FoxCS100
  # glm_X_FoxCS100 = glm(BADGERns ~  # GLM without FoxCS100
  #                        GDN100 + ALT1000 + AMN1000 + PLPK500 +
  #                        WAT1000 + DTCH1000 + TERR1000 + 
  #                        poly(logHD1000, 2) + logTRAF1000 + HogCS100, 
  #                       family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  # auc_change_GLMmod[10, i] <- auc(eval$BADGERns, predict(glm_X_FoxCS100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  # 
  # 
  # # 11 -- GLM without HogCS100
  # glm_X_HogCS100 = glm(BADGERns ~  # GLM without BadgCS250
  #                        GDN100 + ALT1000 + AMN1000 + PLPK500 +
  #                        WAT1000 + DTCH1000 + TERR1000 + 
  #                        poly(logHD1000, 2) + logTRAF1000 + FoxCS100, 
  #                       family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  # auc_change_GLMmod[11, i] <- auc(eval$BADGERns, predict(glm_X_HogCS100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  # 
  # 
}

# best_mod_AUC <- as.numeric(auc_GLMmod['GLM_Qua_selected_AUC', 'Mean']) # store final threshold value
best_mod_AUC <- 0.7307753 # calculated in step 2 script
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Badger/badger_AUCchange_map.pdf", width=10, height=7.5, units="in", scale = 1)


auc_change_summary <- auc_change_GLMmod[,c("Predictor", "Mean", "AUC_Difference")]
write.csv(auc_change_summary, '../../../results/CitiSci/AllYear_XMammalPred/Badger/badger_AUCchange_summary.csv')

##############################################################
#### Visualize response curves of GLM Qua using ggeffects ####
##############################################################
enviro$AMN1000 <- enviro$AMN1000 * 100 # x100 to convert into % for variables with 0-1 range
enviro$WD1000 <- enviro$WD1000 * 100
# enviro$FoxCS100 <- enviro$FoxCS100 * 100
# enviro$GDN100 <- enviro$GDN100 * 100 
...
gge_all <- rbind(gge_AMN1000,  gge_DTCH1000, gge_WD1000, 
                 gge_logHD1000, gge_logTRAF1000 
                 )
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Badger/badger_predictor_ggplot.pdf", width=10, height=7.5, units="in", scale = 1)
