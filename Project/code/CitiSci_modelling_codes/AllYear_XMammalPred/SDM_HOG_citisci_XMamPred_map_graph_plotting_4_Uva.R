...
write.csv(enviroplot, "../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/enviroplot.csv")

ggsave("../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/hog_pre_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/hog_CI_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/hog_bi_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
auc_change_GLMmod <- as.data.frame(matrix(0, ncol = 100, nrow = 10, # create dataframe to store AUC outputs
                                   dimnames = list(c('GDN100', 'ALT1000', 'PLPK100', 
                                                     'WAT100', 'WD100', 'poly(IMP100)','DTCH100',
                                                     'TERR750', 'poly(logHD100)','TRAF100'
                                                     ), NULL)))
...
for (i in 1:100){  # i = 100 repeat split sample cross validation
  ...
  a <- SampleMat2(ref = enviro$HOGns, ratio = 0.75) # 70/30 split for calibration and evaluation
  ...
  glm_X_PLPK100 = glm(HOGns ~  # GLM without PLPK100
                        ...
                        poly(logHD100, 2) + TRAF100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[3, i] <- auc(eval$HOGns, predict(glm_X_PLPK100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 4 -- GLM without WAT100
  glm_X_WAT100 = glm(HOGns ~  # GLM without WAT100
                       GDN100 + ALT1000 + PLPK100 + 
                       WD100 + poly(IMP100, 2) + DTCH100 + TERR750 + 
                       poly(logHD100, 2) + TRAF100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[4, i] <- auc(eval$HOGns, predict(glm_X_WAT100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 5 -- GLM without WD100
  glm_X_WD100 = glm(HOGns ~  
                      GDN100 + ALT1000 + PLPK100 + 
                      WAT100 + poly(IMP100, 2) + DTCH100 + TERR750 + 
                      poly(logHD100, 2) + TRAF100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[5, i] <- auc(eval$HOGns, predict(glm_X_WD100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 6 -- GLM without poly(IMP100, 2)
  glm_X_polyIMP100 = glm(HOGns ~  
                           GDN100 + ALT1000 + PLPK100 + 
                           WAT100 + WD100 + DTCH100 + TERR750 + 
                           poly(logHD100, 2) + TRAF100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[6, i] <- auc(eval$HOGns, predict(glm_X_polyIMP100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 7 -- GLM without DTCH100 
  glm_X_SEMI750 = glm(HOGns ~  
                        GDN100 + ALT1000 + PLPK100 + 
                        WAT100 + WD100 + poly(IMP100, 2) + TERR750 + 
                        poly(logHD100, 2) + TRAF100, 
                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[7, i] <- auc(eval$HOGns, predict(glm_X_SEMI750, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # # 9 -- GLM without SEMI750 
  # glm_X_SEMI750 = glm(HOGns ~  
  #                       GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
  #                       poly(IMP100, 2) + DTCH100 + TERR750 + 
  #                       poly(logHD250, 2) + BadgCS250 + FoxCS250, 
  #                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  # auc_change_GLMmod[9, i] <- auc(eval$HOGns, predict(glm_X_SEMI750, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  # 
  # 
  # 8 -- GLM without TERR750
  glm_X_TERR750 = glm(HOGns ~  
                        GDN100 + ALT1000 + PLPK100 + 
                        WAT100 + WD100 + poly(IMP100, 2) + DTCH100 +
                        poly(logHD100, 2) + TRAF100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[8, i] <- auc(eval$HOGns, predict(glm_X_TERR750, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  
  # 9 -- GLM without poly(logHD100, 2) +
  glm_X_polylogHD100 = glm(HOGns ~  
                             GDN100 + ALT1000 + PLPK100 + 
                             WAT100 + WD100 + poly(IMP100, 2) + DTCH100 + TERR750 + 
                             TRAF100, 
                     family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[9, i] <- auc(eval$HOGns, predict(glm_X_polylogHD100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # 10 -- GLM without TRAF100 +
  glm_X_TRAF100 = glm(HOGns ~  
                             GDN100 + ALT1000 + PLPK100 + 
                             WAT100 + WD100 + poly(IMP100, 2) + DTCH100 + TERR750 + 
                             poly(logHD100, 2), 
                           family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[10, i] <- auc(eval$HOGns, predict(glm_X_TRAF100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  
  
  # # 12 -- GLM without BadgCS250
  # glm_X_BadgCS250 = glm(HOGns ~  # GLM without BadgCS250
  #                         GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
  #                         poly(IMP100, 2) + DTCH100 + SEMI750 + TERR750 + 
  #                         poly(logHD250, 2) + FoxCS250, 
  #                    family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  # auc_change_GLMmod[12, i] <- auc(eval$HOGns, predict(glm_X_BadgCS250, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  # 
  # # 13 -- GLM without FoxCS250
  # glm_X_BadgCS250 = glm(HOGns ~  # GLM without BadgCS250
  #                         GDN100 + ALT1000 + AMN750 + PLPK100 + WAT100 + WD100 + 
  #                         poly(IMP100, 2) + DTCH100 + SEMI750 + TERR750 + 
  #                         poly(logHD250, 2) + BadgCS250, 
  #                       family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  # auc_change_GLMmod[13, i] <- auc(eval$HOGns, predict(glm_X_BadgCS250, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  # 
  
}

# best_mod_AUC <- as.numeric(auc_GLMmod['GLM_Qua_selected_AUC', 'Mean']) # store final threshold value
best_mod_AUC <- 0.7634524
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/hog_AUCchange_map.pdf", width=10, height=7.5, units="in", scale = 1)


auc_change_summary <- auc_change_GLMmod[,c("Predictor", "Mean", "AUC_Difference")]
write.csv(auc_change_summary, '../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/hog_AUCchange_summary.csv')
...
gge_DTCH100 <- ggpredict(glm_qua_selected_step,
                         ...
                         typical = 'mean', back.transform = T)
gge_DTCH100$Predictor <- "Detached housing (%)"




# # SEMI750  
# gge_SEMI750   <- ggpredict(glm_qua_selected_step, 
#                            terms = c('SEMI750   [all]'), ci.lvl = 0.95, 
#                            typical = 'mean', back.transform = T)
# gge_SEMI750$Predictor <- "Semi-detached housing (%)" 
...
gge_TRAF100 <- ggpredict(glm_qua_selected_step,
                         ...
                         typical = 'mean', back.transform = T)
gge_TRAF100$Predictor <- "Traffic volume"
...
gge_all <- rbind(gge_ALT1000, gge_GDN100, gge_PLPK100, gge_WAT100, gge_WD100,
                 gge_IMP100, gge_DTCH100, gge_TERR750, gge_logHD100, gge_TRAF100
                 )
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Hedgehog/hog_predictor_ggplot.pdf", width=10, height=7.5, units="in", scale = 1)
