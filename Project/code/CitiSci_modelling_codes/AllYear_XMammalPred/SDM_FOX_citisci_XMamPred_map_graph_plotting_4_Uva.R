...
final_threshold <- 0.4213000 # calculated in step 2 script
...
write.csv(enviroplot, "../../../results/CitiSci/AllYear_XMammalPred/Fox/enviroplot.csv")
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_pre_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_CI_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_bi_map.pdf", width=10, height=7.5, units="in", scale = 1)
...
auc_change_GLMmod <- as.data.frame(matrix(0, ncol = 100, nrow = 9, # create dataframe to store AUC outputs
                                          dimnames = list(c('GDN1000', 'ALT750', 'CEM500','PLPK100',
...
for (i in 1:100){  # i = 100 repeat split sample cross validation
  ...
  glm_X_PLPK100 = glm(FOXns ~  # GLM without PLPK100
                        GDN1000 + ALT750 + CEM500 + WD1000 + 
                        poly(IMP750, 2) + DTCH1000 + SEMI500 + logTRAF1000, 
                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[4, i] <- auc(eval$FOXns, predict(glm_X_PLPK100, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
  ...
  glm_X_DTCH1000 = glm(FOXns ~  
                         GDN1000 + ALT750 + CEM500 + PLPK100 + WD1000 + 
                         poly(IMP750, 2) + SEMI500 + logTRAF1000, 
                       family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  ...
  glm_X_SEMI500 = glm(FOXns ~  
                        GDN1000 + ALT750 + CEM500 + PLPK100 + WD1000 + 
                        poly(IMP750, 2) + DTCH1000 +logTRAF1000, 
                      family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  ...
  # 9 -- GLM without logTRAF1000
  glm_X_logTRAF1000 = glm(FOXns ~  # GLM without logTRAF1000
                            GDN1000 + ALT750 + CEM500 + PLPK100 + WD1000 + 
                            poly(IMP750, 2) + DTCH1000 + SEMI500, 
                          family='binomial'(link = 'logit'), data = calib) #  use calib dataset to built the model
  auc_change_GLMmod[9, i] <- auc(eval$FOXns, predict(glm_X_logTRAF1000, eval, type='response')) # use eval dataset to predict to -- auc(response, predictor)
...
best_mod_AUC <- 0.6621181 # calculated in step 2 script
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_AUCchange_map.pdf", width=10, height=7.5, units="in", scale = 1)


auc_change_summary <- auc_change_GLMmod[,c("Predictor", "Mean", "AUC_Difference")]
write.csv(auc_change_summary, '../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_AUCchange_summary.csv')
...
enviro$CEM500 <- enviro$CEM500 * 100
...
# CEM500
gge_CEM500 <- ggpredict(glm_qua_selected_step,
                         terms = c('CEM500 [all]'), ci.lvl = 0.95,
                         typical = 'mean', back.transform = T)
gge_CEM500$Predictor <- "Cemetery cover (%)"
...
gge_all <- rbind(gge_GDN1000, gge_ALT750, gge_CEM500, gge_PLPK100, gge_WD1000,
                 gge_IMP750, gge_DTCH1000, gge_SEMI500, gge_logTRAF1000
)
...
ggsave("../../../results/CitiSci/AllYear_XMammalPred/Fox/fox_predictor_ggplot.pdf", width=10, height=7.5, units="in", scale = 1)
