...
question_8 <- function() {
  ...
  plot(abundance ~ seq(length(abundance)), xlab = "Time(Generation)", ylab = "Species Richness", pch = 1, cex = 0.35,cex.main = 0.6, main = "Change in species richness over 200 generations in a neutral model")
...
question_16 <- function()  {
  ...
  barplot(plot_matrix, names.arg = df$group, beside = TRUE, col = c("skyblue", "pink"), ylim=range(pretty(c(0, plot_matrix))),
          cex.main = 0.8, main = "Species abundance distribution at equilibrium with different initial maximum and minimum number of species", 
  ...
  legend("topright", legend=c("Initial community with maximum 100 species", "Initial community with minimum 100 species"), 
         fill = c("skyblue", "pink"), cex=0.8, bty = "n")
...
process_cluster_results <- function() {
  ...
  for (i in 1:100){
    file <- paste0("Neutral_cluster_simulation_", i, ".rda")
    load(file,.GlobalEnv)
  ...
  for (a in 1:25){
    ...
    no_gen_postburnin_com500 = max_500 - 81 # number of generations after burn-in
  ...
  for (a in 76:100){
    ...
    for (b in 21:max_5000){
      octave_sum_com5000 = sum_vect(octave_sum_com5000, unlist(octave_all_simu[[a]][[b]])) 
    }
  }

  
  ############ average of all octaves
  vector_com500 = octave_sum_com500 / sum(total_com500) # divided by the number of octave intervals
  vector_com1000 = octave_sum_com1000 / sum(total_com1000)
  vector_com2500 = octave_sum_com2500 / sum(total_com2500)
  vector_com5000 = octave_sum_com5000/ sum(total_com5000)
...
plot_cluster_results <- function()  {
  ...
  barplot(unlist(combined_results[[1]]), 
          cex.main = 0.8,
          main = "Equilibrium state in community size = 500", ylim = c(0, 3.5),
          xlab = "Number of individuals per species", ylab = "Number of species",
  ...
  barplot(unlist(combined_results[[2]]), 
          cex.main = 0.8,
          main = "Equilibrium state in community size = 1000", ylim = c(0, 8),
          xlab = "Number of individuals per species", ylab = "Number of species",
  ...
  barplot(unlist(combined_results[[3]]), 
          cex.main = 0.8,
          main = "Equilibrium state in community size = 2500", ylim = c(0, 20),
          xlab = "Number of individuals per species", ylab = "Number of species",
  ...
  barplot(unlist(combined_results[[4]]), 
          ...
          xlab = "Number of individuals per species", ylab = "Number of species", ylim = c(0, 35),
          names.arg = c("1", "2-3", "4-7", "8-15", "16-31", "32-63", "64-127", "128-255", "256-511", "512-1023", "1024-2047"))

    print(combined_results) # return results output for each community
}
...
draw_tree <- function()  {
  ...
  tree(c(2,0), pi/2, 0.5) # suitable parameters to draw a tree
}
...
draw_fern <- function()  {
  graphics.off() # clear any existing graphs 
  plot(1, type="n", xlab="", ylab="", xlim=c(0, 5), ylim=c(0, 5)) # create empty plot
  fern(c(2,0), pi/2, 0.5) # suitable parameters to draw a fern
}
...
draw_fern2 <- function()  {
  graphics.off() # clear any existing graphs 
  plot(1, type="n", xlab="", ylab="", xlim=c(0, 5), ylim=c(0, 5)) # create empty plot
  fern2(c(2,0), pi/2, 0.5, 1) # suitable parameters to draw a fern
}
